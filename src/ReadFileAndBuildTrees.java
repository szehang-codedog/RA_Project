...
public class ReadFileAndBuildTrees {
	...
	public void readFileLines(String content) {
		int BEGIN_LINE = 9; // the first expected output line
		 ...
		 * 
		 * for (String ruleLine : rules) { //System.out.println(rule);
		 * rule.add(ruleLine.split(":")); }
		 */

		// System.out.println(rule.get(0)[0]);System.out.println(rule.get(0)[1]); //path
		// and similarity
		// System.out.println(rule.get(1)[0]);System.out.println(rule.get(1)[1]); //path
		// and unorder

		/*
		 * int ruleId = 0; for (String rule : rules) {
		 * ruleLevel.add(rule.trim().split(",")[0]);
		 * ruleLevelId.add(rule.trim().split(",")[1]);
		 * matchingRule.add(rule.trim().split(",")[2]);
		 * System.out.println(ruleLevel.get(ruleId)+" "+ ruleLevelId.get(ruleId) +" "+
		 * matchingRule.get(ruleId)); ruleId++; }
		 */
		...
		for (String rule : rules) {
			if (rule.startsWith("s")) {
				if (rule.trim().split("=").length == 2) {
					similarity = Double.parseDouble(rule.trim().split("=")[1].split(":")[0]);
					sim_path = new int[rule.trim().split("=")[1].split(":")[1].split(",").length];
					for (int i = 0; i < rule.trim().split("=")[1].split(":")[1].split(",").length; i++) {
						sim_path[i] = Integer.parseInt(rule.trim().split("=")[1].split(":")[1].split(",")[i]);
					}
				}
			}
			...
			if (rule.startsWith("u")) {
				if (rule.trim().split("=").length == 2) {
					unorder = new int[rule.trim().split("=")[1].split(":")[0].split(",").length];
					unorder_path = new int[rule.trim().split("=")[1].split(":")[1].split(",").length];
					for (int i = 0; i < rule.trim().split("=")[1].split(":")[0].split(",").length; i++) {
						unorder[i] = Integer.parseInt(rule.trim().split("=")[1].split(":")[0].split(",")[i]);
					}
					for (int i = 0; i < rule.trim().split("=")[1].split(":")[1].split(",").length; i++) {
						unorder_path[i] = Integer.parseInt(rule.trim().split("=")[1].split(":")[1].split(",")[i]);
					}
				}
			}
		}
	...
	public void traverseNCompare(TreeNode<String> expRoot, TreeNode<String> actRoot) {
		if (expRoot.level.equalsIgnoreCase("L0") && actRoot.level.equalsIgnoreCase("L0")) {
			...
			if (expRoot.type.equals("DOUBLE") && actRoot.type.equals("DOUBLE")) {
				...
				if (!matched) {
					distinctOutputs.add(currentFileName + ";" + tcid + ";" + seq + ";" + expRoot.data.trim() + ";"
							+ actRoot.data.trim() + ";" + "numeric not match");
				}
				...
				if (expRoot.data.trim().split("\\s").length > 1 || actRoot.data.trim().split("\\s").length > 1) // if either string has more than one elements, NLP
				{
					...
					List<NLPTreeNode<String>> expNLPTree = core.parseSentence(expRoot.data.trim()); // Whole expected NLP Tree
					List<NLPTreeNode<String>> actNLPTree = core.parseSentence(actRoot.data.trim()); // Whole actual NLP Tree


					/*
					String expLeafNodes = core.findSubtreeByPath(expNLPTree, sim_path);
					String actLeafNodes = core.findSubtreeByPath(actNLPTree, sim_path);

					if(expLeafNodes != null && actLeafNodes != null){
						two_phrase_sim = ps.phraseSim(expLeafNodes.trim(), actLeafNodes.trim()); //Actually compare the similarity of two desired subtree
					}

					if (two_phrase_sim >= similarity)
						matched = true;
					else
						matched = false;

					if (debugLevel >= 1 && (expLeafNodes != null && actLeafNodes != null)) {
						System.out.println("(Similarity) " + "\"" + (expLeafNodes.trim()  + "\"" + " vs "  + "\"" + actLeafNodes.trim()  + "\"" + " => "
								+ two_phrase_sim));
					}
					...
					if (!matched) {
						distinctOutputs.add(currentFileName + ";" + tcid + ";" + seq + ";" + expRoot.data.trim() + ";"
								+ actRoot.data.trim() + ";" + "NLP Sim: " + two_phrase_sim);
					}
					 */
				} else {
		...
		distinctOutputs.add(
				currentFileName + ";" + tcid + ";" + seq + ";" + ";" + ";" + "subtree not symmetric 1 (seem match)");

		// traverse child nodes
		if (expRoot.children.size() != actRoot.children.size()) {
			System.err.println("Two subtrees are not symmetric - failed to compare");
			distinctOutputs.add(
					currentFileName + ";" + tcid + ";" + seq + ";" + ";" + ";" + "subtree not symmetric 2 (not match)");
		} else {
	...
	private void buildOutputBlocks(StringBuilder exp, StringBuilder act) {
		...
		if (expLines.length != actLines.length) {
			...
			distinctOutputs
					.add(currentFileName + ";" + tcid + ";" + seq + ";" + ";" + ";" + "number of row are different");
			return;
		}
		...
		if (debugLevel >= 1) {
			System.out.println("*(L0" + " level:" + eRoot.level + " levelID:" + eRoot.levelID
					+ ") build tree root for the expected output: ");
		}
		// read each block
		...
		if (debugLevel >= 1) {
			System.out.println("\nTraverse trees and compare nodes " + "Seq=" + seq);
		}
	...
	private TreeNode<String> buildTreeForGeneralBlock(String blockOfLines, TreeNode<String> root, int blockID) {
		...
		if (debugLevel >= 1) {
			System.out.println("|-(L1" + " level:" + blockRoot.level + " levelID:" + blockRoot.levelID
					+ ") build subtree for a general block: ");
		}
		int lineID = 0;
		...
		for (String line : lines) {
			...
			if (debugLevel >= 1) {
				System.out.println(" |-(L2" + " level:" + lineRoot.level + " levelID:" + lineRoot.levelID
						+ ") build subtree for a line: ");
			}
			String[] tokens = line.split("[:]");
			int tokenId = 0;
			for (String token : tokens) {
				TreeNode<String> tokenRoot = lineRoot.addChild(token.trim(), getType(token.trim()), "L3", tokenId++, -1,
						-1, getRule(token.trim()));
				if (debugLevel >= 1) {
					System.out.println("  |-(L3" + " level:" + tokenRoot.level + " levelID:" + tokenRoot.levelID
							+ ") add node: " + token.trim());
				}
			}
		}
	...
	private TreeNode<String> buildTreeForFormattedBlock(String blockOfLines, TreeNode<String> root, int blockID) {
		...
		for (int i = 0; i < rowCount; i++) {
			for (int j = 0; j < colCount - 1; j++) {
				if (delimiterList.contains(matrix.outputMatrix[i][j])
						|| delimiterList.contains(matrix.outputMatrix[i][j + 1])) {
					colAdjs[j]++;
				}
			}
		}
		...
		for (int i = 0; i < rowCount - 1; i++) {
			for (int j = 0; j < colCount; j++) {
				if (delimiterList.contains(matrix.outputMatrix[i][j])
						|| delimiterList.contains(matrix.outputMatrix[i + 1][j])) {
					rowAdjs[i]++;
				}
			}
		}
	...
	// a pre-process that converts lines of outputs to a character matrix (each line
	// is padded to the maximum width)
	public Matrix genOutputMatrix(String outputLines) {
		// find the max width of the matrix (consider a calendar, this first row and the
	...
	public void writeDistinctMismatchedOutputs(File file) {
		...
		try {
			...
			bw.write("pid;tcid;seq;expected;actual;error");
			bw.newLine();
	...
	public static void main(String[] args) {
		...
		//String outputFolderPath = "C:\\Users\\CTC\\Desktop\\RAProject\\outputs_20171219\\";
		String outputFolderPath = "C:\\Users\\lamsz\\OneDrive - The Hang Seng University of Hong Kong\\RA\\outputs_20171219\\";
		// String outputFolderPath = "C:\\Users\\Administrator\\Desktop\\output1\\";

		// modify this line to change result file directory
		//String resultFilePath = "C:\\Users\\CTC\\Desktop\\RAProject\\";
		String resultFilePath = "C:\\Users\\lamsz\\OneDrive - The Hang Seng University of Hong Kong\\RA\\";

		final String[] pids = { "pid_682_Lab05_Q01", // 0
				"pid_683_Lab05_Q03_dayOfWeek", // 1
				"pid_1477_Lab08_Q02_VendMach", "pid_1514_Lab13_Q3a_leastCoins", "pid_1516_Lab13_Q3b_listLeastCoins",
				"pid_1550_Lab01_Q05_HelloWrd123", "pid_1552_Lab01_Q06_HelloWlcm123", "pid_1567_Lab02_Q01(c)Mid",
				"pid_1568_Lab02_Q02(b)Oranges", "pid_1569_Lab02_Q03_CredUnits", "pid_1574_Lab02_Q05", // 10
				"pid_1576_Lab02_Q06(a)", "pid_1578_Lab02_Q06(b)", "pid_1579_Lab02_Q07(b)", "pid_1611_Lab03_Q01",
				"pid_1613_Lab03_Q02", "pid_1614_Lab03_Q03_triangle", "pid_1618_Lab03_Q04(b)Score_chk",
				"pid_1620_Lab03_Q05", "pid_1621_Lab03_Q06", "pid_1622_Lab03_Q07(a)Hex2Dec_2Digits", // 20
				"pid_1623_Lab03_Q07_Hex2Dec_00toFF", "pid_1648_Lab04_Q04(a)_MulTable",
				"pid_1649_Lab04_Q04(b)_MulTable_Framed", "pid_1654_Lab04_Q06_TableOfPowers",
				"pid_1661_Lab05_Q02_MarksArray", "pid_1662_Lab05_Q04_nFunctions", "pid_1664_Lab05_Q05",
				"pid_1688_Test1_CA1_Q1", "pid_1702_Test1_C61_Q4_CntDivisor", "pid_1708_Lab06_Q01", // 30
				"pid_1721_Lab06_Q03", "pid_1729_Lab07_Q01", "pid_1734_Lab07_Q03", "pid_1744_Lab07_Q04",
				"pid_1749_Lab07_Q05", "pid_1757_Lab08_Q01_DateV3", "pid_1758_Lab08_Q03_Search2DArr",
				"pid_1759_Lab08_Q04", "pid_1760_Lab08_Q05", "pid_1761_Lab08_Q06_CompareAlphaCh", // 40
				"pid_1764_Lab08_Q08_ReverseDigits", "pid_1765_Lab08_Q09_ClassifyMarks", "pid_1777_Lab09_Q01",
				"pid_1778_Lab09_Q02", "pid_1779_Lab09_Q03", "pid_1781_Lab09_Q04", "pid_1784_Lab10_Q02",
				"pid_1785_Lab10_Q03", "pid_1787_Lab10_Q04", "pid_1790_Lab11_Q01_first_n_abc", // 50
				"pid_1793_Lab11_Q03_Add_big_num_arr", "pid_1794_Lab11_Q04_Add_big_num_class",
				"pid_1803_Lab12_Q03_StudentResult", "pid_1805_Lab13_Q01a_containEven", "pid_1806_Lab13_Q01b_showRev",
				"pid_1807_Lab13_Q01c_containDigit", "pid_1808_Lab13_Q01d_countDigits",
				"pid_1809_Lab13_Q01e_leftMostDigit", "pid_1811_Lab13_Q02_binSearch_recursive",
